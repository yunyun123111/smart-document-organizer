...
class TestFutureMigration:
    ...
    def test_new_migration_applied_on_legacy(self, tmp_path, monkeypatch):
        """老库升级：新增 v2 迁移会被执行，且只执行一次。"""
        eng, Session = _make_session(tmp_path)
        # 模拟老库：业务表 + legacy baseline（版本1）
        with Session() as db:
            db.execute(text("CREATE TABLE documents (id INTEGER PRIMARY KEY, name TEXT)"))
            db.commit()
            database.run_migrations(db)  # 打 legacy baseline

        # 模拟发布新版本：SCHEMA_VERSION=2，新增 v2 迁移
        monkeypatch.setattr(database, "SCHEMA_VERSION", 2)
        monkeypatch.setattr(
            database, "MIGRATIONS",
            [(2, "add demo col", self._add_col_fn("demo_col"))],
        )
        with Session() as db:
            database.run_migrations(db)
            assert (2, "add demo col") in _migration_names(db)
        ...
        with Session() as db:
            database.run_migrations(db)
            assert _migration_names(db).count((2, "add demo col")) == 1
        eng.dispose()

    def test_new_migration_skipped_on_fresh(self, tmp_path, monkeypatch):
        """新库：create_all 已是最新结构，v2 迁移直接标记已应用、不执行。"""
        monkeypatch.setattr(database, "SCHEMA_VERSION", 2)
        monkeypatch.setattr(
            database, "MIGRATIONS",
            [(2, "add demo col", self._add_col_fn("demo_col"))],
        ...
        with Session() as db:
            database.run_migrations(db)
            # 新库 fresh baseline=SCHEMA_VERSION=2 → v2 被逻辑标记为已应用，不执行
            assert (0, "fresh baseline") in _migration_names(db)
            assert (2, "add demo col") not in _migration_names(db)
